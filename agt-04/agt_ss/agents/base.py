...
agt_ss.agents.base
-------------------
Abstract base class for all AGT-SS sub-agents.

Each agent implements:
  - can_run(state)  → bool          — dependency check before invocation
  - run(state)      → WorkflowState — executes the agent's processes, returns updated state

Agents must NOT mutate state in place; they return a new dict merged by the orchestrator.
Tool failures raise AgentToolError, which the orchestrator catches and routes to dead-letter.
"""
...
class BaseAgent(ABC):
    ...
    def _audit(self, state: dict, action: str, detail: str = "") -> None:
        ...
        # Non-destructive: log is collected by orchestrator on merge
